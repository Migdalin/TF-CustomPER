...
class DdqnAgent():
    def __init__(self, agentParams, action_size, batchHelper, progressTracker):
        ...
        self.trainingModel = self.BuildModel('training')
    ...
    def Replay(self):
        ...
        # Note how we are passing ones as the mask.
        ...
        # The Q values of the terminal states are 0 by definition, so override them
        next_Q_values[gameOvers] = 0
        
        # The Q value of each start state is the reward + gamma * the max next state Q value
        Q_values = rewards + (self.Params.gamma * np.max(next_Q_values, axis=1))

        # Note how we are passing the actions as the mask and multiplying
